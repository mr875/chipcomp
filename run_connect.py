...
def snpid_swapin(curs,uid_to_swapout,db_snp,old_ds,new_ds,this_altid=None):
    ...
    if this_altid:
        curs.execute("INSERT INTO alt_ids (id, alt_it, datasource) VALUES (%s, %s, %s)",(db_snp,this_altid,new_ds))
...
for line in reader.linebyline():
    ...
    if dbsnpid:
        ...
        if snp_exist:
            if secondary_id != dbsnpid: #snpid found but if sec id is new add to alt_ids
                add_altid(curs,dbsnpid,secondary_id,new_ds)
        elif secondary_id != dbsnpid: #if the uid contains a non dbsnp id
